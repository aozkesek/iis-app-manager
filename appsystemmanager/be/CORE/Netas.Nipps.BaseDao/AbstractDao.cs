...
{
    public abstract class AbstractDao<T, C> : IGenericDao<T> where C : DbContext
    {
        ...
        public abstract IQueryable<T> ListTQueryByName(C dbContext, string name);
        ...
        public virtual List<T> ListByCategory(string category)
        {
            using (C dbContext = NewDbContext())
            {
                IQueryable<T> records = ListTQueryByName(dbContext, category);
                GetTLogger().Debug("{0} of {1} listed by {2}.", PageSize, records.Count(), category);
                if (PageSize > 0)
                    return records.Take(PageSize).ToList();
                else
                    return records.ToList();
